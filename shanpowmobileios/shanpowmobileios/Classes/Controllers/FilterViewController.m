...
- (void)viewDidLoad {
	...
	self.view.backgroundColor = UIC_BRIGHT_GRAY(1.0);
}
	...
	switch (indexPath.section) {
		...
		case 1:
		{
			if (self.lastChannelCell) {
				self.lastChannelCell.accessoryView = nil;
			}
			UITableViewCell *currentCell = [tableView cellForRowAtIndexPath:indexPath];
			UIImageView *checkmark = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"checkmark"]];
			currentCell.accessoryView = checkmark;
		...
		case 2:
		{
			if (indexPath.row == 0) {
				...
				[UIView animateWithDuration:0.25 animations: ^{
				    currentCell.backgroundColor = UIC_ALMOSTWHITE(0.2);
				}];
			}
			break;
		}
		...
		case 3:
		{
			if (self.lastScoreCell) {
				self.lastScoreCell.accessoryView = nil;
			}
			UITableViewCell *currentCell = [tableView cellForRowAtIndexPath:indexPath];
			UIImageView *checkmark = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"checkmark"]];
			currentCell.accessoryView = checkmark;
	...
	switch (indexPath.section) {
		...
		case 2:
		{
			switch (indexPath.row) {
				case 0:
				{
					...
					if (cell == nil) {
						cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
						cell.backgroundColor = UIC_ALMOSTWHITE(0.2);
					}
				...
				case 1:
				{
					...
					bkView.backgroundColor = UIC_ALMOSTWHITE(1.0);
		...
		case 3:
		{
			...
			if (cell == nil) {
				...
				if (indexPath.row == 0 || (indexPath.row - 1 == self.scoreToShow && self.scoreToShow != 0)) {
					UIImageView *checkmark = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"checkmark"]];
					cell.accessoryView = checkmark;
