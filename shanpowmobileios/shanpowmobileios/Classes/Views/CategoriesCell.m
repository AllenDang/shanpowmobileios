	...
	if (self) {
		...
		for (int i = 0; i < 20; i++) {
			...
			btn.frame = CGRectMake(0.0 + (width / 4) * (i % 4),
			                       ...
			                       (width / 4) - (IsSysVerGTE(7.0) ? 0.5 : 1),
			                       self.cellHeight - (IsSysVerGTE(7.0) ? 0.5 : 1));
...
- (void)categorySelected:(UIButton *)sender {
	...
	[self setCategorySelected:sender];
...
- (void)setCategorySelected:(UIButton *)button {
	if (self.lastSelectedButton) {
		[UIView animateWithDuration:0.1
		                 animations: ^{
		    self.lastSelectedButton.backgroundColor = UIC_BRIGHT_GRAY(1.0);
	...
	[UIView animateWithDuration:0.25
	                 animations: ^{
	    button.backgroundColor = (IsSysVerGTE(7.0) ? UIC_BRIGHT_GRAY(0.5) : UIC_ALMOSTWHITE(0.5));
	}
